...
const LandingPage = () => {
  ...
  useEffect(() => {
    // Get stored leaderboard or use mock data
    const storedLeaderboard = localStorage.getItem("quizLeaderboard")
    if (storedLeaderboard) {
      setPlayers(JSON.parse(storedLeaderboard))
    } else {
      setPlayers(mockPlayers)
      localStorage.setItem("quizLeaderboard", JSON.stringify(mockPlayers))
    }
  }, [])
            ...
            <li key={index} className="player-item">
              <div className="player-info">
                ...
                <div className="player-avatar">
                  <img src={player.avatar || "/placeholder.svg"} alt={`${player.name}'s avatar`} />
                </div>
